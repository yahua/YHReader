...
- (void)updateProgress {
    [(AWMBRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
        self.indicator = [[[AWMBRoundProgressView alloc] init] autorelease];
    }
...
//         UIWindow *window = [[ViewChangeManager sharedManager] keyWindow];
//         parentView = window;
	}
	id me = [self initWithFrame:parentView.bounds];
...
@implementation AWMBRoundProgressView
